...
function App() {
  ...
  return (
    ...
    <TextField id="newtodo" variant="outlined" label="Ny oppgave" />
    <List>
      {todos.map((todo, index) => (
        todo.completed === false ?
        <ListItem key={index}>
          <ListItemIcon>
            <Checkbox check={todo.completed} onChange={() => (updatetodo(index))}/>
          </ListItemIcon>
    ...
    <Typography variant="h" component="h2">Completed</Typography>
    <List>
      {todos.map((todo, index) => (
        todo.completed === true ?
        <ListItem key={index}>
